(function () {
  ...
  function urlFor(key) {
    ...
    return `https://embed.waze.com/tr/iframe?zoom=${zoom}&lat=${lat}&lon=${lon}`;
  }
  function render() {
    if (!enabled) {
    ...
    frame.title = "Waze canlı trafik haritası – " + buttons.find(item => item.dataset.location === current).textContent.trim();
