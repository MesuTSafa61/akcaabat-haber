(function () {
  ...
  async function updateRoadworks() {
    ...
    try {
      const response = await fetch(config.url + "/functions/v1/roadworks", {
        headers: { apikey: config.key }, signal: AbortSignal.timeout(15000)
      });
      if (!response.ok) throw new Error("unavailable");
  ...
  document.getElementById("trafficRefresh").addEventListener("click", () => { render(); updateEtas(); updateRoadworks(); });
  ...
  setInterval(() => { if (!document.hidden) updateRoadworks(); }, 30 * 60_000);
